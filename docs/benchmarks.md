---
description: Compare measured aube, pnpm, npm, Yarn, Bun, and Deno performance with explicit fixtures, cache conditions, and reproduction steps.
---
...
These measurements describe this fixture, tool versions, hardware, and cache
state. They do not establish that one tool is fastest for every project.
The script scenario uses a no-op test command, so it measures command overhead,
not the time needed to run a real test suite. Each row scales its bars
independently; compare the printed times across scenarios.

The README and landing page read the same checked-in results. Their benchmark
version can be older than the current aube release; the legend above records
the versions that were actually measured.
...
fixed 500 Mbit/s bandwidth and 50 ms latency. This reduces variation from the
network; CPU, filesystem, and machine load can still affect the results.
The benchmark harness serializes runs with `/tmp/aube-bench.lock`:
