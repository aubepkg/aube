...
must be explicitly allowlisted before their install-time scripts run. `esbuild`
is trusted by default because its installer selects the platform-specific binary;
an explicit deny rule still disables it.
