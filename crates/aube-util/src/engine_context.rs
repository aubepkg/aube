...
//! binary. But an embedder also computes a handful of values *per project /
//! per invocation* — which override sources apply, whether a pnpm-named file
//! is the active PM's and may be read, the PATH/env overlay that routes
//! lifecycle scripts through a provisioned runtime. A compile-time const
//! cannot carry those. [`EngineContext`] is their home: a process-global
...
pub struct EngineContext {
    ...
    /// `pnpm_config_*` / `PNPM_CONFIG_*` (its general settings env) and
    /// ignores `npm_config_*`. An embedder whose active PM is provably
    /// **pnpm v11+** sets this `true` so the registry-client track mirrors
    /// pnpm 11; under any earlier pnpm, any other PM, or standalone aube it
    /// stays `false`.
    ///
    /// Scope note: this gates ONLY the registry-client track. The GENERIC
    /// settings family has its own `pnpm_config_*` aliases gated separately
    /// by [`read_branded_pnpm_config`](Self::read_branded_pnpm_config).
    ...
    /// from `.npmrc` and takes every layout/behavior setting from
    ...
    /// `.npmrc` matches pnpm 11; under any earlier pnpm, any other PM, or
    /// standalone aube it stays `false`.
    ...
    /// behavior. An embedder sets this only when Yarn is the active incumbent;
    /// under another PM, Yarn-named config is another tool's state and must
    /// not be read.
    ...
    /// preserves upstream aube behavior. An embedder sets this only when Bun
    /// is the active incumbent; under another PM, Bun-named config is another
    /// tool's state and must not be read.
    ///
    /// Bun's semantics (mirrored here): `BUN_CONFIG_REGISTRY` sets the default
    /// registry and is checked *before* `NPM_CONFIG_REGISTRY` /
    /// `npm_config_registry` (so it outranks them); `BUN_CONFIG_TOKEN` sets
    /// the default registry's auth token, checked before `NPM_CONFIG_TOKEN` /
    ...
    /// owns them. An embedder sets this only under its own project identity,
    /// so root-native config is not accidentally honored while mirroring
    ...
    /// through a provisioned/augmented runtime. Default empty =
    /// behavior-preserving.
    ...
    /// produced no switch, the runtime spawn helpers fall back to this so a
    /// fetched-bin `exec node` shebang, a dlx/create child, and node-program
    ...
    /// create) never touch. This reaches those paths through the runtime
    /// helpers instead. `None` (default) = upstream behavior — the resolver
    /// owns node discovery — byte-identical for standalone aube.
    ...
    /// [`runtime_node_bin`](Self::runtime_node_bin) — an embedder that owns
    /// Node provisioning already holds it, so aube neither probes nor
    /// re-derives it. `engines::effective_node_version` consults this at the
    /// resolver's tier: below an explicit `node-version` `.npmrc` override
    /// (pnpm's validation-only knob), above the ambient `node --version` PATH
    /// probe. `None` (default) keeps the PATH probe — upstream behavior.
    pub runtime_node_version: Option<String>,

    /// Replacement lifecycle `npm_config_user_agent` product token. `None`
    /// (default) falls back to the compile-time
    /// [`Embedder::user_agent`](crate::identity::Embedder::user_agent) —
    /// standalone aube reports `aube/<version>`. An embedder sets `Some` when
    /// the product string is genuinely *runtime*: a per-mode UA embedding the
    /// project's RESOLVED node version, which can't be a compile-time literal.
    /// Read at the lifecycle-UA seam in `aube-scripts`.
    ...
    /// activating it for standalone aube, breaking default-preservation.
    ...
    /// applies per active PM. Same shape and contract as
    /// [`embedder_overrides`].
    ///
    ...
    ///
    /// When `true`, the embedder additionally (1) stamps the checksum onto its
    /// own generic lockfile (the `Aube` kind, pnpm-v9 bytes) so the drift
    /// check reaches a fixpoint, and (2) treats a stored-vs-computed checksum
    /// mismatch as lockfile drift — a normal install re-resolves, a
    /// `--frozen-lockfile` install aborts — mirroring pnpm's
    /// `ERR_PNPM_LOCKFILE_CONFIG_MISMATCH`. Harmless under lockfile kinds that
    /// carry no checksum (npm/yarn/bun): stored and computed both resolve to
    /// `None`, so the check is a no-op.
